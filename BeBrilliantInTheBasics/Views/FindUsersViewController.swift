...
class FindUsersViewController: UIViewController {
    ...
    private func addOrRemoveUserToFirebase(userId: String, email: String, isAdding: Bool) {
        ...
        if isAdding {
            // Implement your Firebase function to add user to the list
            db.collection("users").document(userId).collection("friends").whereField("email", isEqualTo: email).getDocuments { [self] snapshot, error in
                if let error = error {
                ...
                db.collection("users").document(userId).collection("friends").addDocument(data: ["email": email]) { error in
                    if let error = error {
                        print("Error adding user: \(error.localizedDescription)")
                    } else {
            ...
            // Implement your Firebase function to remove user from the list
            db.collection("users").document(userId).collection("friends").whereField("email", isEqualTo: email).getDocuments { [self] snapshot, error in
                if let error = error {
                ...
                for document in documents {
                    db.collection("users").document(userId).collection("friends").document(document.documentID).delete { error in
                        if let error = error {
                            print("Error removing user: \(error.localizedDescription)")
                        } else {
...
extension FindUsersViewController: UISearchBarDelegate {
    ...
    private func isUserAlreadyAdded(email: String, completion: @escaping (Bool) -> Void) {
        ...
        db.collection("users").document(currentUser!.uid).collection("friends").whereField("email", isEqualTo: email).getDocuments { snapshot, error in
            if let error = error {
